...
(#:key inputs #:allow-other-keys)
                   (substitute* (list "test-dat.in" "test-strings.in")
		                        (("(basename|cmp|diff|mktemp|rm|sed|seq)"
cmd)
        ...
        (native-search-paths
	     (list (search-path-specification
				     ...
				     (define-public fortunes-jkirchartz
				       ;; No public release.
				         ;; Note to updaters: Please
ensure that new quotes do not bring harm
  ;; rather than fortune.
...
"daikichi pack"))))
 ...
 (add-after 'build 'check
               ...
               invoke
	                               (search-input-file inputs
"libexec/daikichi/test-dat")
                        (find-files "." "\\.dat$"))))))))
			      (home-page "https://github.com/JKirchartz/fortunes")
			            (synopsis "Collection of fortunes")
				          (description "This package
contains a large collection of quotes to
display via @command{fortune}, drawn from sources all around the
world.")
...
"censor" dest)))))))
    (home-page "https://github.com/jaseg/lolcat")
	    ...
	    (description "@command{lolcat} concatenates files and
streams like
...
"oneko-remove-nonfree-characters.patch"))
	              ...
	              ;; Remove bitmaps with copyright issues.
		              '(begin
			                 (for-each delete-file-recursively
					                      (cons*
"bitmaps/bsd" "bitmaps/sakura" "bitmaps/tomoyo"
...
_
             (invoke "xmkmf")
	                  ;; Fix incorrectly generated compiler flags.
			               (substitute* "Makefile"
				                      (("(CDEBUGFLAGS
= ).*" _ front) (string-append front "-O2\n")))))
         (replace 'install
...
,version))
                    (man (string-append out "/share/man"))
		                        (man6 (string-append man "/man6"))
					                    (man6-ja
(string-append man "/ja/man6")))
               (install-file "oneko" bin)
	                      (mkdir-p man6)
			                     (mkdir-p man6-ja)
					                    (copy-file
"oneko.man" (string-append man6 "/oneko.6"))
...
(git-file-name name version))
       (sha256
               (base32
"1zrfd71zx2px2xpapg45s8xvi81xii63yl0h60q72j71zh4sif8b"))))
    (build-system gnu-build-system)
        (inputs
	     (list ncurses))
	         (arguments
...
"http://www.tkl.iis.u-tokyo.ac.jp/~toyoda/index_e.html")
    (synopsis "Joke command to correct typing \"sl\" by mistake")
        (description
	     "@dfn{SL} (for Steam Locomotive) displays one of several
animated trains
...
"1gaigpda1w9wxfh8an3sam1hpacc1bhxl696w4yj0vzhc6izqvxs"))
       (modules '((guix build utils)))
              (snippet '(begin
	                         ;; kenny is under nonfree Artistic
License (Perl) 1.0.
                   (delete-file "kenny")
		                      (substitute* "Makefile"
				                           (("kenny")
    ...
    (native-inputs
	     ...
	     (inputs
		      ...
		      (home-page "https://joeyh.name/code/filters/")
			      ...
			      (description
			           "The filters collection harks back
to the late 1980s, when various text
...
@item spammer: turn honest text into something liable to be flagged as
spam
...
nethackify, pirate
           license:gpl3+          ; scramble, scottish
	              license:public-domain  ; jethro, kraut, ken, studly
		                 license:gpl1+          ; cockney,
jive, nyc only say "gpl"
...
".tar.gz"))
       (sha256
               (base32
"17pxc955jgkjan8ax0lw3b3sibw7aikc7p9qbxsp0w7g7jkxf666"))))
    (build-system gnu-build-system)
        (arguments
	     '(#:phases
	            (modify-phases %standard-phases
		             (add-before 'configure 'fix-install-path
			                (lambda _
					             ;; Install
program to bin instead of games.
             (substitute* "src/Makefile.in"
	                    (("(gamesdir =
\\$\\(exec_prefix\\)/)games" _ prefix)
			         ...
			         (inputs
				      (list gtk+ libx11 libxpm libxt libxml2))
				          (native-inputs
					       (list pkg-config))
					           (home-page
"https://www.ratrabbit.nl/ratrabbit/xsnow/index.html")
    (synopsis "Let it snow on the desktop")
        (description "@code{Xsnow} animates snowfall and Santa with
reindeer on
the desktop background.  Additional customizable effects include wind,
stars
...
"src/nyancat" bin)
               (install-file "nyancat.1" man))
				    ...
				    (synopsis "Nyan cat telnet server")
				        (description
					     "This is an animated,
color, ANSI-text telnet server that renders a loop
	           ...
	           (native-inputs
			    ...
			    (inputs
				     ...
				     (home-page "https://gitlab.com/jallbrit/cbonsai")
				         (synopsis "Grow bonsai trees
in a terminal")
    (description "Cbonsai is a bonsai tree generator using ASCII art.
It
